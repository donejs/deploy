...
var copyFiles = function(directory, files) {
	console.log("Copy:")
	files.forEach(function(location) {
		try {
			fsx.copySync(location, directory);
		} catch (e) {
...
module.exports = function(directory, files) {
	...
	if (!files) {
		...
		var packageJSON =
			JSON.parse(fs.readFileSync(packagePath + "/package.json", {
				encoding: "utf8"
			}));

		if (!packageJSON.files) {
			throw new Error("files to copy must be provided on the "
